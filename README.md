...
- Chunked prefill is gated on the model's declared capability, not on a
  ...
  plugin instead rounds `max_num_batched_tokens` and
  `long_prefill_token_threshold` down to multiples of the model's declared
  `chunked_prefill_token_alignment`, so the served token budget may be slightly
