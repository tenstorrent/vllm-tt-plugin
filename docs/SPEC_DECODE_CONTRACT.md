...
9. Release. When `SchedulerOutput.finished_req_ids` contains A,
   ...
   **`TTModelRunner._release_model_request`**, which calls **the adapter's**
   ...
   if the adapter defines that hook (section 4g). **The adapter** releases
   A's model-owned drafter state inside `release_request`; an adapter without
   that hook is never told that A is gone.
