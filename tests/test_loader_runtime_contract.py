...
def test_loader_forwards_vllm_config_to_explicit_runtime_abi(monkeypatch):
    """An explicitly declared config parameter is the generated-model ABI."""
    received = {}
    sentinel = object()
    ...
    model_config = SimpleNamespace(hf_config=object(), max_model_len=131072)
    monkeypatch.setattr(
        "vllm_tt_plugin.loader.get_model_architecture",
        lambda _model_config: (GeneratedModel, None),
    )
